...
func CreateDepartment(c *gin.Context) {
	...
	dept := models.Department{Name: req.Name, Remark: req.Remark}
...
func DeleteDepartment(c *gin.Context) {
	...
	// 5) 删除职位
	if err := tx.Where("department_id = ?", id).Delete(&models.Position{}).Error; err != nil {
		tx.Rollback()
		c.JSON(http.StatusInternalServerError, gin.H{"code": 1, "msg": "删除部门职位失败"})
		return
	}
