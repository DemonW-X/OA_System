    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑菜单' : '新增菜单'" width="520px">
      <el-form :model="form" label-width="96px">
        ...
        <el-form-item label="业务编码">
          <el-input v-model="form.biz_code" placeholder="如 leave_request、event_booking；留空表示不关联审批流" />
        </el-form-item>
...
const form = ref({ id: null, name: '', icon: 'Menu', path: '', parent_id: null, sort_code: 0, visible: true, remark: '', biz_code: '', biz_name: '', biz_sort: 0 })
...
const openDialog = (row = null) => {
  if (!row) {
    form.value = { id: null, name: '', icon: 'Menu', path: '', parent_id: 0, sort_code: 0, visible: true, remark: '', biz_code: '', biz_name: '', biz_sort: 0 }
  } else {
    form.value = {
      id: row.id,
      name: row.name,
...
const handleSubmit = async () => {
  ...
  const hasBizCode = !!String(form.value.biz_code || '').trim()
  const hasBizName = !!String(form.value.biz_name || '').trim()
  if (hasBizCode !== hasBizName) {
    return ElMessage.warning('业务编码和业务名称需同时填写或同时留空')
  }
  const payload = {
    ...
    biz_code: String(form.value.biz_code || '').trim(),
    biz_name: String(form.value.biz_name || '').trim(),
